...
Implementation status: complete. The checked items identify code, tests, and CI
gates now present in the repository; they do not assert that a particular
release-evidence run passed. Before strengthening release claims, record a
successful qualifying run using the checklist at the end of this document.

## 1. Make the test environment part of the evidence

- [x] Add a Windows test helper that resolves the volume containing the actual
...
- [x] Make contract tests fail or explicitly skip with a recorded reason unless
...
- [x] Include the resolved test-root evidence in the Windows CI artifact rather
  than recording only workspace and runner-wide volume listings.
- [x] Keep `windows-2022` required and run `windows-latest` before release. Pin
...
- [x] Add a child-process stress harness with no cooperative pause around
...
- [x] After every kill, open a fresh store and require the canonical path to be
...
- [x] Run separate create and replace campaigns. Replacement is the critical
  contract path; creation ensures the absent-to-present transition is covered.
- [x] Use payloads spanning zero length, one byte, the streaming chunk
...
- [x] Record the random seed, parent kill delay, child iteration counter,
...
- [x] Run enough bounded repetitions to make the syscall-overlap window
...
- [x] Refactor the Windows save wrapper behind test-only injectable operations
...
- [x] For every injected pre-replacement failure, assert that the old canonical
...
- [x] For every injected replacement failure, assert only states justified by
  the returned Win32 error. Treat unproven cases as ambiguous and reload before
  deciding whether a retry is safe.
- [x] Test `ERROR_SHARING_VIOLATION` with a destination handle opened without
...
- [x] Inject namespace-directory open and flush failures after a successful
...
- [x] Add equivalent streaming cases for early EOF, trailing input, input I/O
...
- [x] Hold a read handle opened with `FILE_SHARE_DELETE`, replace the blob, and
  demonstrate that the old handle can retain the old file object while a fresh
  open sees the new canonical file.
- [x] Hold incompatible read, write, mapping, and executable-image handles where
...
- [x] Stress independent store instances and external writers targeting the
...
- [x] Retain same-process, same-store FIFO tests to show that strengthening
...
- [x] Isolate `sync_windows_directory` in native tests and record behavior on
...
- [x] Add a test proving that directory-flush failure is surfaced and cannot be
  mistaken for an atomic replacement failure.
- [x] Review authoritative Microsoft documentation periodically. Unless it
...
- [x] Do not add destructive power-cut testing to ordinary CI. If VM or physical
...
- [x] Replace the README's prose support boundary with a concise Windows support
...
- [x] Keep crate-level docs, README, release checklist, and error documentation
  aligned on three separate concepts: complete staging, canonical-name
  visibility, and persistence requests.
- [x] Link test artifacts or a release evidence summary from `RELEASE.md` without
  claiming that stress testing proves a universal Win32 guarantee.
- [x] Require the environment check, deterministic failure suite, replacement
...
- [x] Revisit the wording only after those gates pass. Even then, scope the
...
- [ ] Record the successful `windows-2022` 10,000-attempt soak artifact URL and
  confirm every contract-test environment report says `local_ntfs=true`.
- [ ] Record the successful `windows-latest` 2,000-attempt compatibility
  artifact, exact toolchain versions, attempts, and completed replacements.
- [ ] Confirm the deterministic failure suite, ordinary Windows suite, and
  extracted-package consumers passed in the same workflow run.
- [ ] Keep the release wording scoped to trusted local NTFS, one coordinated
  writer, process interruption, and empirical evidence rather than power-loss
  or universal Win32 guarantees.
